...
import numpy as np
import cv2 as cv
from utils import euclid_distance, check_collision, draw_car, draw_angled_rec, draw_arrow
from utils import readMapFromFile, normalize, normalizeTheta, centerToXY, move, move_backward
...
class State:
    ...
    def __init__(self, car, parent=None):
        ...
        super().__init__()
        self.car = car
        self.position = car.getCenter()
        self.parent = parent
        self.g = 0
        self.h = 0
        self.f = 0
        self.carInfo = car.getInfo()
        self.carCenter = car.getCenter()

    def compute_dist(self, dubins):
        ...
        self.h = dubins.getShortestPath(self.carInfo)
    ...
    def __hash__(self):
        ...
        """
        return hash(normalize(self.carInfo))

    def isGoal(self, goal):
        """Check whether a state is the goal.
        """
        return euclid_distance(self.carCenter, (goal[0], goal[1])) < 30 \
            and normalizeTheta(self.car.theta) == normalizeTheta(goal[2])
...
class Map:
    ...
    def draw(self, state, start, goal):
        ...
        while state is not None and state != start: 
            cv.circle(img, state.car.getCenter(), 2, BLACK, thickness=-1)
            img_tmp = np.array(img)
            draw_arrow(img_tmp, *goal)
...
class PathFinding:
    ...
    def __init__(self, map, start, goal):
        ...
        self.dubins = Dubins(centerToXY(*self.goal))
    ...
    def solve(self):
        ...
        while self.min_heap:
            ...
            for neighbor in neighbors:
                ...
                neighbor.compute_dist(self.dubins)
        ...
        print("No solutions")
        print(currState.getCar().getCenter())
        return False


if __name__ == '__main__':
    img, grid, start, goal = readMapFromFile(args.map)
    car = Car(*start)
    start = State(car)
    map = Map(img, grid)
    ...
    # print(centerToInverse(*goal))
    # draw_angled_rec(img, *centerToInverse(*goal))
    # cv.imshow(window, img)
    # cv.waitKey(0)
    # cv.destroyAllWindows()
